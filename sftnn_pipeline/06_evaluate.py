...
def get_sftnn_probs(test_df, feature_cols, n_features, n_regions, artifacts_dir):
    ...
    if "region_alpha" in state_dict:
        # Experiment #13: checkpoint was trained with the fixed per-region
        # dampening buffer -- register a same-shaped buffer so strict
        # loading succeeds and the saved alpha values are actually restored
        # (not just skipped), since alpha affects inference behavior.
